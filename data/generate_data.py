...
def generate_user_data(n):
    users = []
    used_emails = set()  # Set to store used emails
...
def generate_product_data(n):
    products = []
    used_names = set()  # Set to store used product names
    categories = {
    ...
    while len(products) < n:
        ...
        if name not in used_names:
            used_names.add(name)
            category = random.choice(["Electronics", "Accessories", "Wearable Tech", "Photography", "Home Appliances"])
            description_phrases = random.sample(categories[category], 3)  # Select 3 random features
            description = f"This {category.lower()} product offers {description_phrases[0]}, {description_phrases[1]}, and {description_phrases[2]}."
            product = {
...
def generate_sales_data(n, users, product_ids):
    ...
    for _ in range(n):
        ...
        sale = {
            ...
            "product_id": random.choice(product_ids),  # Assuming product_ids is defined
...
def generate_feedback_data(n, users, product_ids):
    ...
    # Categorizing adjectives by sentiment
